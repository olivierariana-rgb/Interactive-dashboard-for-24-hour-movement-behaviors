...
# IMPORTANT: your data uses 'Adult' (not 'Adults')
age_choice = st.radio(
    "Select age group:",
    ["All", "Children", "Adolescents", "Adult"],
    horizontal=True
)
...
if n_studies == 0:
    ...
    method_vars = {
        ...
        "Device Model": "Device_Model",  # added
    ...
    for label, col in method_vars.items():
        ...
        # OPTIONAL: brand -> model breakdown (only if both exist)
        if (col == "Device_Brand") and ("Device_Model" in meta_age.columns):
            st.markdown("**Device models used within each brand:**")
            for brand, brand_n in counts.items():
                st.markdown(f"*{brand}* (n = {brand_n})")
                sub_brand = meta_age[meta_age["Device_Brand"] == brand]

                model_counts = sub_brand["Device_Model"].fillna("NR").value_counts()

                for model, m_cnt in model_counts.items():
                    pct_m = round(100 * m_cnt / brand_n, 1) if brand_n else 0
